...
- `L(jw) = C(e^{jwTs}) * G(jw)`
- Для parallel / effective-ideal зворотного зв'язку:
  - `C(e^{jwTs}) = Kp + Ki*Ts/(1 - z^{-1}) + Kd*(1 - z^{-1})/Ts * alpha/(1 - (1 - alpha)z^{-1})`
- Для SERIES зворотного зв'язку:
  - `C(e^{jwTs}) = Kc*(1 + Ts/(Ti*(1 - z^{-1})))*(1 + Td*(1 - z^{-1})/Ts * alpha/(1 - (1 - alpha)z^{-1}))`
- `z = e^{jwTs}`
- Примітки:
  - Об'єкт залишається ідентифікованою неперервною FOPDT-моделлю `G(jw)`.
  - Частина регулятора використовує ту саму дискретну структуру D-фільтра, що й прошивка.
